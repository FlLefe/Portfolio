...
function ProjectDetails({ project, onBack }) {
    ...
    return (
        <div className='sm:h-screen w-full bg-gradient-to-b from-slate-900 via-slate-950 to-slate-800 text-white px-12 pt-24'>
            ...
            <h2 className='text-4xl m-4 font-bold inline border-b-4 border-gray-500'>{t('ProjectsDetails.title')}</h2>
                ...
                <button onClick={onBack} className="text-white w-fit px-6 py-3 my-2 flex items-center font-bold rounded-md bg-gradient-to-r from-cyan-500 to-blue-500 cursor-pointer hover:text-orange-300">{t('ProjectsDetails.button')}</button>
            </div>
        </div>
    );
}
